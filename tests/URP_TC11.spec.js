...
test.describe('Registration Page Test', () => {
    ...
    test('Verify Last Name Field Correctly Rejects Arithmetic Symbols', async ({ page }) => {
    const locators = await fieldsLocators(page);

    //Enter valid First name
    await locators.firstName.fill(user.first_name[0]);
    //Enters invalid Last name
     await locators.lastName.fill(user.last_name[5]);

     // Handle the dialog that appears when the Last name field is filled with Arithmetic Symbols
